...
class MigrationGenerator extends Generator {
    ...
    protected function getTemplate($template, $name)
    {
        ...
        $module = $this->needsModule($template);


        $upMethod = $this->getUpStub($module);
        $downMethod = $this->getDownStub($module);
    ...
    */
    protected function parseMigrationName($name)
    {
        ...
        return array($action, strtolower($tableName));
    }
    ...
    * @return string
    */
    protected function getUpStub($module=FALSE)
    {
        switch($this->action) {
            case 'add':
            case 'insert':
            ...
            default:
                ...
                if ($module){
                    ...
                    $model = lcfirst($this->cache->getModelName());

                    $upMethod.= $this->file->get(__DIR__ . '/templates/migration/migration-up-insert-perms.txt');
                    $upMethod = str_replace('{{ModuleName}}', $moduleName, $upMethod);
                    $upMethod = str_replace('{{model}}', $model, $upMethod);
                }
                $fields = $this->fields ? $this->setFields('addColumn') : '';
                break;
        }
    ...
    * @return string
    */
    protected function getDownStub($module=FALSE)
    {
        switch($this->action) {
          case 'add':
          case 'insert':
